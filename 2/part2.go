...
func IsReportSafe(line string, hasAlreadyRecursed bool) (bool, error) {
	...
	// check if removing a level will be ok
	// Check if removing a level will be okay
	if !hasAlreadyRecursed && isUnsafeReport {
		fmt.Println("This is unsafe and has not recursed:", line)
...
func maind2p2() {
	safeReports, err := ReadReportsFromFileLineByLineWithProblemDampener("input")
	if err != nil {
	...
	// Output the results
	fmt.Println("Sum of safe reports: ", safeReports)
}
